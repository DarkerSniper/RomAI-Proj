        ...
        try {
            ...
            if (message.content.startsWith(syntax)) {
                ...
                if (getCommand == "rs" || getCommand == "recent" || getCommand == "r") {
                    ...
                    await message.reply(reply);
                } else if (getCommand == "multi" && message.author.username == 'romdarker') {
                    let warmups = getUsername.includes('/') ? getUsername.split('/')[1] : undefined;

                    var reply = await multiMatch(undefined, client, message, getUsername, warmups);

                    await message.reply(reply);
                } else if (getCommand == "testcharge" && message.author.username == 'romdarker') {
                    await testRankProgress(message, client, getUsername);
