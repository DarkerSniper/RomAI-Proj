...
module.exports = {
    async triosRequest(interaction, client, discordUser, reqUsers, selectedPool, customBO) {
        try {
            ...
            for (let i=0; i<reqUsers.length; i++) {
                ...
                acceptId += `-${parseInt(parseInt(reqUserId)/ 100000)}`;
                declineId += `-${parseInt(parseInt(reqUserId)/ 100000)}`;
            ...
            collector.on('collect', async (inter) => {
                if (inter.customId == acceptId) {
                    ...
                    if (filterIds.every(id => id == 'done')) {
                        ...
                        handleLobby(undefined, undefined, interaction, client, teams, undefined, selectedPool, customBO); 
                    }
                }
